...
2. Set `self.dataset_path` to point to the CSV generated in **Phase 0** (`finetune_csv/data/kronos_combined_1m_training_data.csv`).
3. Set `self.dataset_begin_time` and `self.dataset_end_time` to match the **actual first and last timestamps** in your CSV.
   You can find these values by running:
   ```bash
   python -c "import pandas as pd; df = pd.read_csv('finetune_csv/data/kronos_combined_1m_training_data.csv'); print('Begin:', df['timestamps'].iloc[0][:10]); print('End:', df['timestamps'].iloc[-1][:10])"
   ```
...
class Config:
    def __init__(self):
        ...
        # IMPORTANT: Set these to the actual first/last timestamps in your CSV.
        # Run: python -c "import pandas as pd; df=pd.read_csv(self.dataset_path); print(df['timestamps'].iloc[0], df['timestamps'].iloc[-1])"
        # Current CSV range: 2026-03-21 13:59:00 -> 2026-06-04 13:58:00 (108,000 rows)
